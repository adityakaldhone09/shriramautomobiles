...
// Check if a valid remote DATABASE_URL is configured
...
if (hasValidUrl) {
  try {
    poolInstance = new Pool({ connectionString: rawUrl, connectionTimeoutMillis: 3000 });
  } catch {
    poolInstance = null;
  }
}
...
if (!poolInstance) {
  ...
  rawMemPool.connect = async (...args: any[]) => {
    const client = await origConnect(...args);
    client.query = wrapQuery(client.query.bind(client));
    return client;
  };
  rawMemPool.query = wrapQuery(rawMemPool.query.bind(rawMemPool));

  poolInstance = rawMemPool;

  // Initialize DB schema in memory asynchronously and track promise
  initDbPromise = initMemTables(rawMemPool);
}

async function initMemTables(pool: any) {
  const ddl = `
  ...
  const cleanDdl = ddl.replace(/ON DELETE CASCADE/gi, '').replace(/ON DELETE SET NULL/gi, '');
  const statements = cleanDdl.split(";").map((s) => s.trim()).filter(Boolean);
  for (const statement of statements) {
    try {
      await pool.query(statement);
    } catch (err: any) {
      console.error("In-memory DDL error:", err.message);
    }
  }

  // Bootstrap baseline data from CSV datasets
  try {
    await bootstrapDatabaseData(pool);
  } catch (err: any) {
    console.warn("Bootstrap data warning:", err.message);
  }
}

export const pool = poolInstance;
export const db = drizzle(poolInstance, { schema });
export { isInMemory, initDbPromise };
