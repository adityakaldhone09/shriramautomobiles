...
type AuthUser = { id: number; customerId?: number | null; role: string; phone: string };
...
export function requireRole(...roles: string[]) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user || !roles.includes(user.role)) return res.status(403).json(createResponse(false, 'Insufficient permissions', undefined, 'FORBIDDEN'));
    return next();
  };
}
